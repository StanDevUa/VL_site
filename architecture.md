...
Auth.js, стратегія сесій — **database sessions** (не JWT): сесія зберігається в таблиці `Session`, cookie конфігурується з `domain: '.lemeshko.org'`. Це дозволяє Ресурсам 2–3 на піддоменах (`emo.lemeshko.org`, `pro.lemeshko.org`) читати ту саму сесійну cookie й підтягувати того самого користувача з БД без окремого SSO-протоколу — саме та схема, яку описує ТЗ.
...
Вхід у адмінку — окремий `isAdmin` прапорець на `User` (а не окрема роль з переліку бізнес-ролей, бо адмін лише один — Вікторія, і це не бізнес-роль клієнта). `/admin/*` захищено в `middleware.ts` перевіркою `session.user.isAdmin`.
...
5. **Новини** — список з фільтром за категорією, створення/редагування: заголовок (i18n), короткий опис/excerpt (i18n), текст (i18n), фото (необов'язково), дата, категорія (select із фіксованих значень).
