...
5. **Новини** — список з фільтром за категорією та лічильником знайденого, створення/редагування: заголовок (i18n), короткий опис/excerpt (i18n), текст (i18n), фото (обов'язкове), категорія (select із фіксованих значень), публікація — перемикач "Опублікувати зараз" / "Запланувати на" (дата+час). **Публікація за розкладом реалізована без окремого поля статусу чи cron-задачі**: поле `date` — це і є момент публікації; публічні сторінки фільтрують `date <= now()` на кожному запиті, тож запланована новина сама з'являється в потрібний момент. У списку адмінки заплановані (ще не опубліковані) новини позначені бейджем "Заплановано", але видно їх завжди — на відміну від публічної частини.
